-- Drop the previous signature (return type changed — Postgres rejects
-- in-place replacement when the return columns differ).
drop function if exists public.consume_dual_quota(uuid, text, text, int, int);
